...
//    private static final String BUY_TICKETS = "https://lab3-ticket-service.herokuapp.com/api/v1/tickets";
    ...
    public Object getBonusAccountPrivileges() {
        try {
            String qwe = ((ServletRequestAttributes) Objects.requireNonNull(RequestContextHolder.getRequestAttributes())).getRequest().getHeader("Authorization");
            String username = JWT.decode(qwe.substring(7)).getClaims().get("nickname").asString();
            ...
            List<BonusHistoryDTO> bonusHistoryDTOS = objectMapper.readValue(jsonNode.get("history").toPrettyString(), new TypeReference<>() {
            });
    ...
    public Object findAll(int page, int size) {
        try {
            String qwe = ((ServletRequestAttributes) Objects.requireNonNull(RequestContextHolder.getRequestAttributes())).getRequest().getHeader("Authorization");
            Claim username = JWT.decode(qwe.substring(7)).getClaims().get("nickname");
            HttpClient httpClient = HttpClient.newHttpClient();
            ...
            List<FlightResponseDTO> flightResponseDTOS = objectMapper.readValue(jsonNode.get("items").toPrettyString(), new TypeReference<>() {
            });
    ...
    public Object buyTicket(FlightBuyRequest flightBuyRequest) {
        try {
            String qwe = ((ServletRequestAttributes) Objects.requireNonNull(RequestContextHolder.getRequestAttributes())).getRequest().getHeader("Authorization");
            String username = JWT.decode(qwe.substring(7)).getClaims().get("nickname").asString();
            ObjectMapper objectMapper = new ObjectMapper();
            ...
            BonusResponseDTO bonusResponseDTO = (BonusResponseDTO) getBonusAccountPrivileges();
    ...
    public Object getTicket(UUID ticketUid) {
        try {
            String qwe = ((ServletRequestAttributes) Objects.requireNonNull(RequestContextHolder.getRequestAttributes())).getRequest().getHeader("Authorization");
            String username = JWT.decode(qwe.substring(7)).getClaims().get("nickname").asString();
    ...
    public Object getTickets() {
        try {
            ...
            List<TicketInformationDTO> ticketInformationDTOS = objectMapper.readValue(response.body(), new TypeReference<>() {
            });
    ...
    public Object getUser() {
        try {
            List<TicketInformationDTO> tickets = (List<TicketInformationDTO>) getTickets();
            UserInformationDTO userInformationDTO = new UserInformationDTO();
            userInformationDTO.setTicketInformation(tickets);

            BonusResponseDTO bonusResponseDTO = new BonusResponseDTO();
            bonusResponseDTO = (BonusResponseDTO) getBonusAccountPrivileges();
            BuyTicketPrivilegeDTO buyTicketPrivilegeDTO = new BuyTicketPrivilegeDTO();
    ...
    public Object ticketCancel(UUID ticketUid) {
        try {
            String qwe = ((ServletRequestAttributes) Objects.requireNonNull(RequestContextHolder.getRequestAttributes())).getRequest().getHeader("Authorization");
            String username = JWT.decode(qwe.substring(7)).getClaims().get("nickname").asString();
            String uriStringGetTicket = UriComponentsBuilder.fromUriString(BUY_TICKETS + "/" + ticketUid).toUriString();
                    ...
                    if (bh.getTicketUid().equals(ticketUid)) {
                        if (bh.getOperationType().equals("FILL_IN_BALANCE")) {
                            ...
                            HttpRequest httpRequestBonusReturn = HttpRequest.newBuilder()
                                    ...
                                    .build();
                            httpClientBonusReturn.send(httpRequestBonusReturn, HttpResponse.BodyHandlers.ofString());
                        }
                    }
                }
